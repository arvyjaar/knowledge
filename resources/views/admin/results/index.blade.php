...
@section('content')
    <h3 class="page-title">@lang('quickadmin.result.title')</h3>
    @can('result_create')
    <p>
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($results) > 0 ? 'datatable' : '' }} @can('result_delete') dt-select @endcan">
                <thead>
                    <tr>
                        @can('result_delete')
                        ...
                        <th>@lang('quickadmin.result.fields.title')</th>
                                                <th>&nbsp;</th>
                    ...
                    @if (count($results) > 0)
                        @foreach ($results as $result)
                            <tr data-entry-id="{{ $result->id }}">
                                ...
                                <td field-key='title'>{{ $result->title }}</td>
                                                                <td>
                                    @can('result_view')
