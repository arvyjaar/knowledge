...
@section('content')
    ...
    @can('document_delete')
        ...
        <ul class="list-inline">
            ...
            |
            <li><a href="{{ route('admin.documents.index') }}?show_deleted=1"
                ...
                @if (count($documents) > 0)
                    @foreach ($documents as $document)
                        <tr data-entry-id="{{ $document->id }}">
                            ...
                            <td field-key='changed'>
                                @if($document->changed)
                                    @foreach($document->changed->getMedia('file') as $media)
                                        <p class="form-group">
                                            <a href="{{ $media->getUrl() }}" target="_blank">{{ $document->changed->nr }}
                                                @if($loop->count > 1){{ ' : '.$loop->iteration }}@endif
                                            </a>
                                        </p>
                                    @endforeach
                                @endif
